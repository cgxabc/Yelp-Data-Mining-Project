...
def main(K, numfeatures, sample_file, num_display_words, outputfile):
    ...
    for i, item in enumerate(lda.show_topics(num_topics=K_clusters, num_words=num_display_words, formatted=False)):
        ...
        for term, weight in item[1]:
            output_text.append( str(term) + " : " + str(weight) )
            
    for t in range(lda.num_topics):
        plt.figure()
        plt.imshow(WordCloud(background_color = "white").fit_words(lda.show_topic(t, num_display_words)))
        plt.axis("off")
...
if __name__=="__main__":
    ...
    parser.add_argument('-displayWN', default=100,type=int,
                       help='This option specifies how many words to display for each topic. Default is 100 words for each topic.')
